...
# 2. ULTRA-RARE METAVERSE AI CYBERPUNK STYLING & COLOR-SHIFT ANIMATIONS
theme_css = """
<style>
    @import url('https://fonts.googleapis.com/css2?family=Orbitron:wght@400;600;800&family=Outfit:wght@300;400;500;600&display=swap');

    /* Color-Shifting Neon Animation Keyframes */
    @keyframes neonShift {
        0% { color: #00f2fe; text-shadow: 0 0 10px rgba(0, 242, 254, 0.4); }
        33% { color: #7f00ff; text-shadow: 0 0 10px rgba(127, 0, 255, 0.4); }
        66% { color: #ff007f; text-shadow: 0 0 10px rgba(255, 0, 127, 0.4); }
    ...
    /* Animated Color-Shifting Header */
    .metaverse-header {
        text-align: left;
        margin-bottom: 40px;
        padding-left: 4px;
    ...
    .metaverse-greeting {
        ...
        font-size: 2.8rem;
        font-weight: 800;
        background: linear-gradient(270deg, #00f2fe, #4facfe, #9b51e0, #ff007f);
    ...
    .animated-color-text {
        font-family: 'Orbitron', sans-serif;
        animation: neonShift 6s infinite;
        font-weight: 600;
    }
    ...
    .stButton button {
        ...
        width: 100% !important;
        box-shadow: 0 0 15px rgba(0, 242, 254, 0.04) !important;
        transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1) !important;
    }

    .stButton button:hover {
        background: linear-gradient(135deg, rgba(0, 242, 254, 0.25), rgba(255, 0, 127, 0.25)) !important;
        border-color: #00f2fe !important;
        color: #ffffff !important;
        box-shadow: 0 0 25px rgba(0, 242, 254, 0.35) !important;
    }

    /* Floating Chat Box Input */
    [data-testid="stChatInput"] textarea {
        ...
        box-shadow: 0 12px 35px rgba(0, 0, 0, 0.8), 0 0 20px rgba(0, 242, 254, 0.06) !important;
    }
    ...
    .sidebar-signature {
        ...
        font-weight: 800;
        animation: neonShift 5s infinite;
    }
...
with st.sidebar:
    st.markdown("<div style='font-family: Orbitron; font-size: 1.15rem; font-weight: 800; margin-bottom: 14px; padding-left: 6px;' class='animated-color-text'>🔮 METAVERSE AI</div>", unsafe_allow_html=True)
    
    if not is_logged_in:
        st.write("<span style='font-size: 0.82rem; color: #9ca3af; padding-left: 6px;'>Authenticate to access the matrix.</span>", unsafe_allow_html=True)
        ...
        st.markdown("<div style='font-family: Orbitron; font-size: 0.68rem; text-transform: uppercase; letter-spacing: 1.5px; color: #9ca3af; margin-top: 18px; margin-bottom: 8px; padding-left: 6px;'>Recent Streams</div>", unsafe_allow_html=True)
...
# 4. Neural Preferences & Settings Modals
if is_logged_in and st.session_state.get("show_settings_modal", False):
    with st.container():
        st.markdown("#### Neural Core Configuration")
        # Updated with Gemini 3.1 & 3.5 model selections
        models_list = [
        ...
        selected_model_input = st.selectbox("Select AI Model Core", models_list, index=model_index)
...
if is_logged_in and st.session_state.get("show_brain_modal", False):
    with st.container():
        st.markdown("#### Quantum Memory Core")
...
if is_logged_in and len(current_session_data["messages"]) == 0:
    st.markdown(f"""
        <div class="metaverse-header">
            ...
            <div class="metaverse-subgreeting">What reality shall we compute today?</div>
        </div>
    """, unsafe_allow_html=True)
else:
    st.markdown(f"""
        <div class="metaverse-header">
            <div class="metaverse-greeting" style="font-size: 1.8rem;">Metaverse AI</div>
...
if not is_logged_in:
    st.markdown("""
        <div class="metaverse-auth-card">
            ...
            <div style="font-family: Orbitron; font-size: 1.35rem; font-weight: 800; color: #f3f4f6; margin-bottom: 8px; letter-spacing: 0.5px;" class="animated-color-text">METAVERSE AI</div>
            <div style="font-size: 0.92rem; color: #9ca3af; line-height: 1.6; margin-bottom: 30px;">
                Connect your Google identity to unlock next-gen Gemini 3.1 artificial intelligence.
            </div>
    """, unsafe_allow_html=True)
...
for message in current_messages:
    with st.chat_message(message["role"]):
        ...
        if "file_name" in message:
            st.markdown(f"<span style='font-size:0.75rem; color:#00f2fe;'>📎 Attached Artifact: {message['file_name']}</span>", unsafe_allow_html=True)

# 6. File Upload Terminal
uploaded_file = st.file_uploader("Upload document, media, or code artifact", type=["png", "jpg", "jpeg", "pdf", "txt", "csv", "py"])

# 7. Prompt Execution & Streaming Pipeline
prompt = st.chat_input("Enter command or prompt into Metaverse AI...")
...
if prompt or uploaded_file:
    ...
    with st.chat_message("user"):
        ...
        if uploaded_file:
            st.markdown(f"<span style='font-size:0.75rem; color:#00f2fe;'>📎 Attached Artifact: {uploaded_file.name}</span>", unsafe_allow_html=True)

    with st.chat_message("assistant"):
        loader_placeholder = st.empty()
        loader_placeholder.markdown("""
            <div class="metaverse-badge animated-color-text">🔮 Metaverse AI is computing...</div>
        """, unsafe_allow_html=True)
        ...
        try:
            ...
            system_instruction = (
                f"You are Metaverse AI, powered by Google's latest Gemini architecture. Respond in {lang_choice}.\n"
